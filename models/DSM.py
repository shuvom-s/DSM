...
def open_and_train(ref_panel, gene_exp, haplos, connectivity_mat, window, size, epochs, testing = False, testing_fake=False, ref_size=1000, negative_examples=None, learning_rate=0.025, device='cpu'):
    # nsnps x nindivs
    # print("opening...")
    # print(window)
    # print(size)
    ...
    PATH = "/broad/cholab/shuvom/HMMs/haplo_models/gtex/chr{}/chr{}_window{}_{}_ref_{}_epochs_{}_lr_{}.pt".format(chrom, chrom, window*size,(window+1)*size, ref_size, epochs, learning_rate)
    print("files ready...") 
    ...
    if not matching:
        ...
        # print(gexp.shape)
        x_onehots = torch.transpose(haplos, 0, 1)[0:1060,window*size:(window+1)*size,:]
        x_onehots_val = torch.transpose(haplos, 0, 1)[1060:1176,window*size:(window+1)*size,:]
        ...
        for i in range(epochs):
            ...
            if i % 5 == 0:
                print("saving...")
                path_prefix = "/broad/cholab/shuvom/HMMs/haplo_models/gtex/chr{}/".format(chrom)
                PATH = path_prefix +  "chr{}_window{}_{}_ref_{}_epochs_{}_lr_{}.pt".format(chrom, window*size, (window+1)*size, ref_size, i, learning_rate)
                torch.save(model.state_dict(), PATH)
                
                p_hmm_val = model.forward_pass_hmm(x_onehots_val)
                val = model.forward(torch.Tensor(gexp_val), x_onehots_val)
                print(val)
                vals.append(float(val.cpu().detach().numpy()))
                dfvals = pd.DataFrame(vals, columns=['Validation Loss (every 5 epochs)'])
                dfvals.to_csv(path_prefix + "chr{}_window{}_{}_ref_{}_epochs_{}_lr_{}_curve_validation.tsv".format(chrom, window*size, (window+1)*size, ref_size,i, learning_rate), index=None, sep='\t')
                print("saving epochs...")
                df = pd.DataFrame(outs, columns=['Loss'])
        
                df.to_csv(path_prefix + "chr{}_window{}_{}_ref_{}_epochs_{}_lr_{}_curve.tsv".format(chrom, window*size, (window+1)*size, ref_size, i, learning_rate), index=None, sep='\t')
                p_hmm = model.forward_pass_hmm(x_onehots)
    
    
    
    
def match_score(refpanel, haplos, gene_exp, connectivity, window, window_size, ref_size=1000, learning_rate=0.025, device='cpu', cache_hmm=True):
    ref = np.load(ref_panel)[window*size:(window+1)*size,0:ref_size]
    assert(ref.shape[1] == ref_size)
    ...
    if cache_hmm:
        p_hmm = model.forward_pass_hmm(x_onehots)
        np.save("/broad/cholab/shuvom/HMMs/haplo_models/gtex/chr{}/chr{}window{}_{}_ref_{}_epochs_{}_lr_{}_p_hmm.npy".format(chrom, chrom, window*size,(window+1)*size, ref_size, epochs, learning_rate), p_hmm.cpu.detach().numpy())
        
    else:
        np.load("/broad/cholab/shuvom/HMMs/haplo_models/gtex/chr{}/chr{}window{}_{}_ref_{}_epochs_{}_lr_{}_p_hmm.npy".format(chrom, chrom, window*size,(window+1)*size, ref_size, epochs, learning_rate), p_hmm.cpu.detach().numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--matching', help='matching')
    ...
    parser.add_argument('--negative_examples', help='negative example haplos', type=int)
    ...
    if args.matching:
        ...
        open_and_train(refpanel, geneexp, eqtls, connectivity, window, window_size, epochs, testing, testing_fake, ref_size, device=device)
